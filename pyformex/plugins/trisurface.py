...
#            cmd += ' -f %s' % fold_smoothing
    ...
    def circularSectionParam(self, method='area'):
        ...
        if method=='area':
            ...
            d = self.getBorderMesh().lengths().sum()/math.pi
        elif method=='hydraulic':
            d = 4.*self.area()/self.getBorderMesh().lengths().sum()
        else:
    ...
    def patchextension(self,p,step,dir=None,makecircular=False,div=1, circulardiameter='area', iter=None):
        ...
        if sum(~in1d(p, self.propSet())):
            raise ValueError,"p should be a valid property number (%s), got %s"%(self.propSet(), p)
        if len(p)>1:
            for i in p:
                self= self.patchextension(i,step,dir,makecircular,div,circulardiameter)
            return self
        s1 = self.selectProp(p)
        c, d1, n1 = circularSectionParam(s1)
        if type(step) == str:
            step = d1*float(step)
        if dir == None:
            dir = n1
        s1x = s1.copy()
        if makecircular:
            s1x = circularizeSurf(s1x, d=circulardiameter, c=c, iter=iter)
        s1x = s1x.translate(dir, step)
        b = s1.border()[0]
        if div == None:
            div = int(ceil(step/b.lengths().mean()))
        bx = s1x.border()[0]
